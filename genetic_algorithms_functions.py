...
#----ADDED_FUNCTION_PMX_CROSS-OVER----
def pmx_crossover(parent1, parent2):
    """
    Partially Mapped Crossover (PMX) for genetic algorithms.

    Parameters:
        - parent1 (list): First parent route (excluding depot 0).
        - parent2 (list): Second parent route (excluding depot 0).

    Returns:
        - list: Offspring route (excluding depot 0).
    """
    size = len(parent1)
    child = [-1] * size  
    ...
    # Create mapping between parent1 and parent2
    mapping = {parent1[i]: parent2[i] for i in range(start, end + 1)}

    # Fill missing slots with parent2 values, resolving conflicts
    for i in range(size):
        if child[i] == -1:  # Empty slot
            candidate = parent2[i]
            while candidate in mapping:  # Resolve conflicts
                candidate = mapping[candidate]
            child[i] = candidate

    return child

#----ADDED_FUNCTION_ADAPTIVE_MUTATE----
